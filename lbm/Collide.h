#ifndef LBM_Collide_H
#define LBM_Collide_H
...
inline void Domain::CollideSRTGamma()
{
    if(Time<0.5) std::cout<<"--- "<<"SRTGA"<<" ---"<<std::endl;    
    size_t nx = Ndim(0);
    size_t ny = Ndim(1);
    size_t nz = Ndim(2);
    #ifdef USE_OMP
    ...
    for (size_t iz=0;iz<nz;iz++)
    {
        if (!IsSolid[ix][iy][iz])
        {
            ...
            for (size_t k=0;k<Nneigh;k++)
            {
                ...
                Ftemp[ix][iy][iz][k] = F[ix][iy][iz][k] - (1-Bn)*NonEq[k]/tau + Bn*Omega + ForceTerm;
...
inline void Domain::CollideMRTGamma()
{
    if(Time<0.5) std::cout<<"--- "<<"MRTGA"<<" ---"<<std::endl;    
    size_t nx = Ndim(0);
    size_t ny = Ndim(1);
    size_t nz = Ndim(2);
    #ifdef USE_OMP
        ...
        if (!IsSolid[ix][iy][iz])
        {
            ...
            while (valid)
            {
                ...
                for (size_t k=0; k<Nneigh; k++)
                {
                    ...
                    double Fvp      = Feq(k    ,rho,VelPt);
                    double Omega    = F[ix][iy][iz][Op[k]] - Fvpp - (F[ix][iy][iz][k] - Fvp);
                    double Noneq = (1-Bn)*fneq[k] - Bn*Omega - ForceTerm;
                    Ftemp[ix][iy][iz][k] = F[ix][iy][iz][k] - alphal*Noneq;
                    if(Ftemp[ix][iy][iz][k]<-1.0e12&&num<2) 
                    {
                        double temp = std::fabs(F[ix][iy][iz][k]/Noneq);
...
inline void Domain::CollideTRTGamma()
{
    if(Time<0.5) std::cout<<"--- "<<"TRTGA"<<" ---"<<std::endl;    
    size_t nx = Ndim(0);
    size_t ny = Ndim(1);
    size_t nz = Ndim(2);
    #ifdef USE_OMP
